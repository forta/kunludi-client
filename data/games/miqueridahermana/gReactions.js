...
móvil.apps.app_reloj: hora del día (empieza con 1020 al llegar a la casa; se incrementa 5 por turno; a las 1380 vas a dormir; te levantas a las 420)
móvil.apps.app_linterna: 0: apagado; 1: encendido
móvil.apps.app_mareas:  hora de la siguiente marea
móvil.apps.app_pilas: carga inicial del móvil (minutos que quedan). Valor inicial: 1440 (100%). Cambios: pierde 5 puntos por turno en modo normal; pierde 120 puntos por turno en modo linterna; gana 75 puntos por turno en modo carga; pierde 45 puntos si llamas.
...
let initReactions =  function  (reactions, primitives) {
		...
		reaction: function (par_c) {
			...
			if (par_c.option == undefined) {
				...
				if (!usr.disponibilidadPNJ (PNJId)) {
					primitives.GD_CreateMsg ("es", "nadie_sale_a_recibirte", "Después de esperar un rato, ves que nadie sale a recibirte. Estarán fuera o muy ocupados.<br/>");
				...
				if (primitives.IT_GetLoc(PNJIndex) ==  primitives.PC_GetCurrentLoc()) {
					...
					primitives.GD_CreateMsg ("es", "DLG_ya_estoy_aquí", "Ya estoy aquí, ¿para qué me llamas?");
				...
				if ((PNJId == "hippie") && (primitives.IT_GetAttPropValue (primitives.IT_X("hippie"), "generalState", "state") != "0")) {
					...
					if (primitives.IT_GetAttPropValue (primitives.IT_X("hippie"), "generalState", "state") == "2") {
						primitives.GD_CreateMsg ("es", "sabes_que_no_te_abrirá", "Tocas el timbre, ves encenderse una luz sobre la cámara... pero sabes que después de la última fiesta salvaje Charlie no te va a abrir durante un tiempo.<br/><br/>");
						primitives.CA_ShowMsg("sabes_que_no_te_abrirá");
						return true
					}

					// aún pendiente de fiesta y setas
					primitives.GD_CreateMsg ("es", "te_observan_por_cámara", "Notas cómo se enciende la luz de la cámara de seguridad y te observan unos segundos. Al final escuchas:<br/>");
					...
					primitives.GD_CreateMsg ("es", "DLG_donde_whisky", "Mi hermano, ¿dónde está el whisky? Creo que te has equivocado de fiesta.");
				...
				primitives.GD_CreateMsg ("es", "%o1_aparece", "%o1 aparece.");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (par_c.loc == primitives.IT_X("estudio")) {
				primitives.GD_CreateMsg ("es", "escribes_en_estudio", "Eres un obseso de la literatura, empiezas retocando una cosita de tu manuscrito y cuando te das cuenta, ya llevas más de una hora con la revisión.<br/>");
				primitives.CA_ShowMsg("escribes_en_estudio");

				usr.incrementar_hora (18) // hora y media: 5 *

				return true
			}

			if (par_c.loc != primitives.IT_X("habitación_gemelos")) {
				primitives.GD_CreateMsg ("es", "sitio_incorrecto_escribir", "Por comodidad no considerar oportuno escribir en otro sitio que no sea en el escritorio de tu antigua habitación.<br/>");
			...
			if (estado_hora.minutosDia >= 360) {
				primitives.GD_CreateMsg ("es", "ya_escribiste_por_hoy", "Ya dedicaste tu ración diaria de escritura. Como tu conciencia está libre para hacer otras cosas, apartas el manuscrito a un lado.<br/>");
			...
			primitives.GD_CreateMsg ("es", "Sesión_escritura", "Te sientas en el escritorio y después de unos momentos de duda, coges carrerilla y cuando te das cuentas ha pasado más de una hora, tiempo en el que reescribes algunas partes de tu nueva novela.<br/>");
			...
			usr.incrementar_hora (18) // hora y media
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if ( (primitives.IT_IsAt(primitives.IT_X("cestaTelePapeo")) != primitives.IT_X("limbo")) && (pedido.length > 0) ) {
				...
				primitives.GD_CreateMsg ("es", "comesTelePapeo", "Es aburrida comida basura, pero tu prioridad en estos días no es comer bien.<br/><br/>");
				primitives.CA_ShowMsg("comesTelePapeo");

				primitives.IT_SetAttPropValue (primitives.IT_X("cestaTelePapeo"), "generalState", "state", "[]")
			} else {

				primitives.GD_CreateMsg ("es", "comesTelePapeoVacío", "Desconsolado, miras las restos que quedan, pero poco más.<br/><br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			primitives.GD_CreateMsg ("es", "perder_tiempo", "La cosa no está como para derrochar el tiempo, pero una cabezadita nunca viene mal... ZZZ.<br/>");
			primitives.CA_ShowMsg ("perder_tiempo");
			usr.incrementar_hora(12) // 1 hora
			return true
		}
		...
		reaction: function (par_c) {
			...
			if (par_c.item2Id == "chimenea") {
				if ((par_c.item1Id != "manuscrito") && (par_c.item1Id != "foto3") && (par_c.item1Id != "foto2")  && (par_c.item1Id != "servilleta")) {
					primitives.GD_CreateMsg ("es", "no_dejar_meter_en_chimenea", "Podría ser interesante hacer que arda... pero no en esta ocasión.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("libro_magia"), "generalState", "state") == "0") {
					if (par_c.item1Id == "foto2") {
						primitives.GD_CreateMsg ("es", "no_dejar_meter_foto2_en_chimenea", "Recuerdas que el libro arcano muestra una imagen de una madre con sus dos hijos.<br/>");
						primitives.CA_ShowMsg ("no_dejar_meter_foto2_en_chimenea");
						return true
					}
					primitives.GD_CreateMsg ("es", "sin_motivo_para_dejar_meter_en_chimenea", "No ves el motivo para meter eso en la chimenea.<br/>");
					primitives.CA_ShowMsg ("sin_motivo_para_dejar_meter_en_chimenea");
					return true
				}

				primitives.GD_CreateMsg ("es", "fantasma_asiente_cuando_metes_en_chimenea", "Una espantosa voz que viene de todos lados y ninguno grita un \'¡sí!\'.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("cargador"), "generalState", "state") != "0") {
					primitives.GD_CreateMsg ("es", "meter_o1_mientras_se_carga_móvil", "¿Meter %o1 mientras el móvil se carga? Va a ser que no.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (primitives.PC_GetCurrentLoc() == primitives.IT_X("cuevita")) {
				primitives.GD_CreateMsg ("es", "atar_cinturón_en_cuevita", "Desde aquí no alcanzas los barrotes, deberás atar el cinturón desde fuera.<br/>");
			...
			if (usr.minutosRestantesMarea() <= 0) {
				primitives.GD_CreateMsg ("es", "atar_cinturón_barrotes_bajo_agua", "Los barrotes están bajo el agua y no consigues atar el cinturón, tendrás que esperar a que baje la marea.<br/>");
				primitives.CA_ShowMsg ("atar_cinturón_barrotes_bajo_agua");
				return true;
			}

			primitives.GD_CreateMsg ("es", "atas_cinturón", "Atas el cinturón a los barrotes de la entrada a la cueva.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (primitives.PC_GetCurrentLoc() == primitives.IT_X("cuevita")) {
				primitives.GD_CreateMsg ("es", "desatar_cinturón_en_cuevita", "Desde aquí no alcanzas los barrotes, deberás desatar el cinturón desde fuera.<br/>");
			...
			if (usr.minutosRestantesMarea() <= 0) {
				primitives.GD_CreateMsg ("es", "desatar_cinturón_barrotes_bajo_agua", "Los barrotes están bajo el agua y no consigues desatar el cinturón, tendrás que esperar a que baje la marea.<br/>");
				primitives.CA_ShowMsg ("desatar_cinturón_barrotes_bajo_agua");
				return true;
			}

			primitives.GD_CreateMsg ("es", "desatas_cinturón", "Desatas el cinturón de los barrotes de la entrada a la cueva.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (par_c.item1Id  == "cuerda_enterrada") {
				primitives.GD_CreateMsg ("es", "coges_collar", "Aparece un collar de conchas como el de tu sueño. No puedes evitar llorar mientras lo acaricias. No puedes explicarlo, pero notas a Ana cerca de ti, como si ella estuviera contigo ahora.<br/>");
		...
		reaction: function (par_c) {
			...
			if ((par_c.item1Id  == "cestaTelePapeo") && (par_c.item2Id  == "embarazada")) {
				...
				if (+primitives.IT_GetAttPropValue (par_c.item2, "generalState", "state") < 3) {
					...
					primitives.GD_CreateMsg ("es", "antisocial", "Por ahora estás muy centrado en tu libro como para hacer vida social, así que al final no lo haces.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (par_c.item1Id  == "móvil") {
				primitives.GD_CreateMsg ("es", "nomobofia", "Cuando vas a dejarlo, te entra tu habitual ataque de nomobofia y no lo sueltas.<br/>");
			...
			if (par_c.item1Id  == "collar") {
				primitives.GD_CreateMsg ("es", "no_dejas_collar", "No te sientes capaz de soltar el collar que te hizo tu querida hermana antes de morir.<br/>");
			...
			if (primitives.PC_GetCurrentLoc() == primitives.IT_X("cuevita")) {
				primitives.GD_CreateMsg ("es", "no_dejas_nada_en_cueva", "Mejor no dejar nada en la cueva, no vaya a ser que suba la marea y lo pierdas.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (+primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas") == 0) {
				primitives.GD_CreateMsg ("es", "sin_baterías", "Las baterías están gastadas. Tendrías que recargarlo primero.<br/>");
				primitives.CA_ShowMsg ("sin_baterías");
				return true;
			}

			if (primitives.PC_IsAt (primitives.IT_X("estudio"))) {
				primitives.GD_CreateMsg ("es", "llamar_desde_estudio", "Ves que en la agenda aparece Freddie, tu vecino de al lado, pero por ahora no ves la necesidad de llamarlo.<br/>");
			...
			if (PNJId == "") {
				if (typeof par_c.option == 'undefined') { // phase 1: asking dialog
					...
					primitives.GD_CreateMsg ("es", "llamar_a_quien", "¿A quién quieres llamar?<br/>");
					primitives.CA_ShowMsg ("llamar_a_quien");

					var menu = []

					primitives.GD_CreateMsg ("es", "cancelar_acción", "Cancelar la acción");
					menu.push ({id:"cancelar", msg:"cancelar_acción"})

					primitives.GD_CreateMsg ("es", "llamar_a_Freddie", "Freddie");
					menu.push ({id:"Freddie", msg:"llamar_a_Freddie"})

					if (primitives.IT_GetAttPropValue (primitives.IT_X("panfleto"), "generalState", "state") != "0") {
						primitives.GD_CreateMsg ("es", "llamar_a_TelePapeo", "TelePapeo");
					...
					if (primitives.IT_GetAttPropValue (primitives.IT_X("hippie"), "generalState", "state") != "0") {
						primitives.GD_CreateMsg ("es", "llamar_a_Charlie", "Charlie");
					...
					if (primitives.IT_GetAttPropValue (primitives.IT_X("embarazada"), "generalState", "state") != "0") {
						primitives.GD_CreateMsg ("es", "llamar_a_Vicky", "Vicky");
						menu.push ({id:"Vicky", msg:"llamar_a_Vicky"})
					}

					primitives.MISC_HideMessages (true);
					primitives.CA_ShowMenu (menu, {type: 'img', isLocal: true, src:"mobile.png", txt:'móvil'}); // continuation in state == 0, phase 2

					return true;

				} else { //second phase:  getting PNJId

					primitives.MISC_HideMessages (false);

					if (par_c.option == "cancelar") {
						primitives.GD_CreateMsg ("es", "no_llamas", "Al final no llamas a nadie");
			...
			if (PNJId ==  "Freddie") usr.menuTelefonoFreddie (par_c.option)
				...
				primitives.GD_CreateMsg ("es", "llamas_a_s1_y_cuelgas", "Llamas a %s1 y cuelgas.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (primitives.PC_GetCurrentLoc() == primitives.IT_X("estudio")) {
				primitives.GD_CreateMsg ("es", "recuerdas_hora_avión", "Recuerdas que deberías salir antes de las 13:00 para el aeropuerto o perderás el avión que te lleva a la costa.<br/>");
			...
			primitives.GD_CreateMsg ("es", "son_las_s1_s2", "Son las %s1:%s2.");
			...
			primitives.GD_CreateMsg ("es", "batería_restante_s1_s2", " Tiempo restante de batería: %s1h%s2m.");
			...
			if (primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_linterna") != "0") {
				primitives.GD_CreateMsg ("es", "linterna_encendida", " Linterna encendida.");
			...
			if (+primitives.IT_GetAttPropValue (primitives.IT_X("hippie"), "generalState", "state") > 0) {
				...
				if (minutos > 0) {
					primitives.GD_CreateMsg ("es", "bajamar_ahora_durante_s1", "La máxima marea baja está siendo ahora y acabará dentro de %s1 minutos.<br/>");
					...
					primitives.GD_CreateMsg ("es", "próxima_bajamar_s1_s2", "La próxima marea baja será a las %s1:%s2.<br/>");
			...
			if (primitives.IT_GetAttPropValue (primitives.IT_X("cargador"), "generalState", "state") == "1") {
				primitives.GD_CreateMsg ("es", "móvil_cargándose", "Se está cargando ahora.<br/>");
				primitives.CA_ShowMsg ("móvil_cargándose");
				if (+primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas") >= 1440) {
					primitives.GD_CreateMsg ("es", "baterías_móvil_llenas", "Las baterías están llenas.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "enciendes_linterna", "Enciendes la linterna del móvil.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "apagas_linterna", "Apagas linterna del móvil<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if ( ! usr.bajoTecho() ){
				primitives.GD_CreateMsg ("es", "sin_enchufe", "Aquí no hay ningún enchufe donde cargarlo.<br/>");
				primitives.CA_ShowMsg ("sin_enchufe");
				return true;
			}

			primitives.IT_SetAttPropValue (primitives.IT_X("cargador"), "generalState", "state", "1")
			primitives.GD_CreateMsg ("es", "lo_pones_a_cargar", "Pones el móvil a cargar, a ver si no tarda mucho.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			primitives.GD_CreateMsg ("es", "retiras_cargador", "Retiras el cargador del móvil.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if (primitives.IT_GetAttPropValue (primitives.IT_X("cargador"), "generalState", "state") != "0") { // móvil cargándose
				primitives.GD_CreateMsg ("es", "no_puedes_salir_mientras_carga", "Intentas moverte... pero ni te separas de tu móvil mientras se está cargando.<br/>");
			...
			if ( ((par_c.loc == primitives.IT_X("salón_comedor")) && (par_c.target == primitives.IT_X("camino_residencial"))) ||
				...
				if (primitives.IT_IsCarried (primitives.IT_X("maleta"))) {
					primitives.GD_CreateMsg ("es", "no_puedes_sacar_maleta_de_casa", "Cuando  te diriges afuera, te sientes un poco ridículo llevando la maleta contigo, así que la dejas antes de salir.<br/>");
				...
				if (primitives.IT_IsCarried (primitives.IT_X("manuscrito"))) {
					primitives.GD_CreateMsg ("es", "no_puedes_sacar_manuscrito_de_casa", "¿Salir de casa con tu valioso manuscrito? Sólo tienes una copia y no te arriesgas a perderlo. Lo dejas en casa antes de salir.<br/>");
			...
			if (+primitives.IT_GetAttPropValue (primitives.IT_X("libro_magia"), "generalState", "state") >= 2) {
				...
				if ( ((par_c.loc == primitives.IT_X("salón_comedor")) && (par_c.target == primitives.IT_X("camino_residencial") || par_c.target == primitives.IT_X("pasillo"))) ||
					...
					primitives.GD_CreateMsg ("es", "fantasma_no_te_deja_ir", "Tu querida hermana no te permite ir en esa dirección.<br/>");
				...
				if (par_c.target == primitives.IT_X("promontorio")) {
					...
					primitives.GD_CreateMsg ("es", "te_despeñas", "<br/><br/>En la noche avanzas siguiendo la voluntad de tu hermana. Al llegar al lugar de las dos lápidas, ves cómo una niebla sale de la tumba de tu madre y se reúne con la figura de tu querida hermana. Ambas te llaman con los brazos abiertos. Caminas hacia ellas como hipnotizado... y de repente sientes que no hay suelo bajo tus pies. Mientras caes, dos luces te acompañan en la caída. Luego, una tercera luz, tú, se reúne con ellas y avanzan juntas hacia un círculo de luz en el horizonte.<br/><br/>")
					primitives.CA_ShowMsg ("te_despeñas");

					primitives.GD_CreateMsg ("es", "fin_juego_despeñas", "El juego ha terminado. ¿Qué habría pasado si hubieras ido a la cueva en vez de al promontorio?<hr/>")
			...
			if (par_c.loc == primitives.IT_X("estudio")) {
				...
				if (primitives.IT_GetLoc(primitives.IT_X("móvil")) != primitives.PC_X()) {
					primitives.GD_CreateMsg ("es", "no_te_olvides_móvil", "¿Irte sin el móvil? Eres un poco rarito y no vas a la última... pero no tanto.<br/>");
					primitives.CA_ShowMsg ("no_te_olvides_móvil");
					return true;
				}

				if (primitives.IT_GetLoc(primitives.IT_X("maleta")) != primitives.PC_X()) {
					primitives.GD_CreateMsg ("es", "no_te_olvides_maleta", "¿Irte de viaje sin  maleta? Eres un poco olvidadizo y despistado, me temo.<br/>");
				...
				if ( (primitives.IT_GetLoc(primitives.IT_X("planta")) == primitives.PC_X()) ||
					...
					primitives.GD_CreateMsg ("es", "cosas_en_mano", "Te piensas salir con las manos tan llenas. Salvo el móvil, sabes que irías más cómodo si llevas el resto de cosas en la maleta.<br/>");
					primitives.CA_ShowMsg ("cosas_en_mano");
					return true;
				}

				if (primitives.IT_GetLoc(primitives.IT_X("planta")) == primitives.IT_X("maleta")) {
					primitives.GD_CreateMsg ("es", "no_te_lleves_planta", "¿Para qué demonios te quieres llevar la planta de viaje?<br/>");
					primitives.CA_ShowMsg ("no_te_lleves_planta");
					return true;
				}

				if (primitives.IT_GetLoc(primitives.IT_X("manuscrito")) != primitives.IT_X("maleta")) {
					primitives.GD_CreateMsg ("es", "no_te_olvides_novela", "Se supone que te vas unas semanas a la casa de la playa a terminar tu NO-VE-LA... ¿no crees que te olvidas algo?<br/>");
			...
			if (!usr.conLuz()) {
				if (par_c.loc == primitives.IT_X("promontorio")) {
					primitives.GD_CreateMsg ("es", "promontorio_sin_luz", "A oscuras, no te atreves a moverte ni un paso por miedo a despeñarte, como le pasó a tu madre.<br/>");
					...
					primitives.GD_CreateMsg ("es", "camino_playa_sin_luz_a_o1", "A oscuras, caminas sin tino y llegas sin saber como a... %o1<br/>");
					...
					primitives.GD_CreateMsg ("es", "caleta_sin_luz_a_camino_playa", "A oscuras, comienzar a subir a tientas un promontorio, pero resbalas y vuelves a donde estabas.<br/>");
			...
			if (par_c.target == primitives.IT_X("cuevita")) {
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("libro_magia"), "generalState", "state") == "2") {
					primitives.GD_CreateMsg ("es", "entras_cueva_con_fanstasma", "Con un inefable sensación de fatídica desgracia entras en la cueva, seguido por tu querida hermana.<br/>");
				...
				if (usr.minutosRestantesMarea() <= 0) {
					primitives.GD_CreateMsg ("es", "cueva_no_accesible", "La pequeña cueva ahora está cubierta por el mar. Tendrás que esperar a la próxima marea baja.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("barrotes"), "generalState", "state") == "0") { // bloqueada la entrada
					primitives.GD_CreateMsg ("es", "cueva_innaccesible", "Los barrotes te impiden entrar en la cuevita.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_linterna") == "0") {
					primitives.GD_CreateMsg ("es", "entrar_sin_linterna", "Está muy oscuro ahí dentro. Como sin luz no verías nada sería tontería entrar ahora.<br/>");
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("cinturón"), "generalState", "state") == "0")  {
					...
					primitives.GD_CreateMsg ("es", "entras_sin_cinturon_atado", "Entras con mucha precaución, atemorizado por lo que le pasó a tu hermana aquí. Seguramente no es una buena idea entrar sin una cuerda o similar atada a los barrotes, por si acaso.<br/>");
			...
			if (par_c.target == primitives.IT_X("caleta")) {
				...
				if (!primitives.IT_GetIsItemKnown(primitives.PC_X(), par_c.target)) {
					primitives.GD_CreateMsg ("es", "primera_vez_caleta", "Avanzas casi arrastrando los pies. No quieres ir, pero no puedes evitarlo. Sabes que tienes que hacer frente al pasado, ya han pasado más de quince años y tienes que superarlo.<br/>");
			...
			if ( (par_c.loc == primitives.IT_X("caleta")) &&
				...
				primitives.GD_CreateMsg ("es", "debes_dejar_cinturón", "No puedes irte con el cinturón atado a los barrores. Lo desatas y te lo llevas contigo.<br/>");
		...
		reaction: function (par_c) {
			...
			if (par_c.item1Id  == "panfleto") {
				...
				if (primitives.IT_GetAttPropValue (par_c.item1, "generalState", "state") == "0") {
					...
					primitives.GD_CreateMsg ("es", "telepapeo_en_agenda", "Añades el teléfono de la tienda TelePapeo en tu agenda del móvil.<br/>");
			...
			if ( (par_c.item1Id  == "barrotes") && (usr.minutosRestantesMarea() <= 0)) {
				primitives.GD_CreateMsg ("es", "ex_barrotes_marea_alta", "Con la marea alta apenas ves unos hierros oxidados bajo el agua. Se pusieron para sellar la entrada a la cueva donde Ana..., cuando la marea está baja.<br/>");
			...
			if (par_c.item1Id  == "cestaTelePapeo") {
				...
				if (pedido.length == 0) {
					primitives.GD_CreateMsg ("es", "CestaTelePapeo_vacía", "La cesta de TelePapeo está vacía.<br/>");
					primitives.CA_ShowMsg ("CestaTelePapeo_vacía");

				} else {
					primitives.GD_CreateMsg ("es", "contenidoCestaTelePapeo", "La cesta de TelePapeo contiene:<br/>");
					primitives.CA_ShowMsg ("contenidoCestaTelePapeo");
					for (var i=0; i<pedido.length; i++) {
						primitives.GD_CreateMsg ("es", "TelePapeo_ver_prod_s1", "%s1<br/>" );
			...
			if (par_c.item1Id  == "libro_magia") {
				...
				if (primitives.IT_GetAttPropValue (par_c.item1, "generalState", "state") == "0") {
					...
					if ( (primitives.IT_GetAttPropValue (primitives.IT_X("embarazada"), "generalState", "state") == "1") ||
						...
						primitives.GD_CreateMsg ("es", "duda_embarazada_gemelos", "Un sombío presentimiento te viene a la cabeza: ¿estará Vicky embarazada de gemelos?<br/>" );
						primitives.CA_ShowMsg ("duda_embarazada_gemelos");
					} else {
						primitives.GD_CreateMsg ("es", "duda_embarazada_gemelos_confirmada", "Algo irrefrenable te anima a seguir adelante con el hechizo, aunque sólo sea para confirmar que no son más que supersticiones sin sentido.<br/>" );
					...
					primitives.GD_CreateMsg ("es", "ex_libro_magia_hechizo2", "ex_libro_magia_hechizo2<br/>" );
					primitives.CA_ShowMsg ("ex_libro_magia_hechizo2");
				} else {
					primitives.GD_CreateMsg ("es", "ex_libro_completo", "El libro no parece tener nada interesante que ofrecerte ahora.<br/>" );
			...
			if (par_c.item1Id  == "cama") {
				if (primitives.IT_GetLoc(primitives.IT_X("diario")) == primitives.IT_X("limbo")) {
					primitives.GD_CreateMsg ("es", "aparece_diario", "Al apoyarte en el colchón para ver cómo está la cama, oyes un ruido de algo que cae al suelo debajo de la cama.<br/>" );
			...
			if (par_c.item1Id  == "diario") {
				if (primitives.IT_GetLoc(primitives.IT_X("foto3")) == primitives.IT_X("limbo")) {
					primitives.GD_CreateMsg ("es", "aparece_foto3", "Entre otras cosas, aparece una foto de tu madre con tu hermana y contigo.<br/>" );
			...
			if (par_c.item1Id  == "espejo") {
				...
				primitives.GD_CreateMsg ("es", "espejo_0", "Por un momento, te parece ver a tu hermana cogida de tu mano al lado tuya. Sientes la mano fría y te la frotas para recuperar el calor.<br/>" );
				primitives.GD_CreateMsg ("es", "espejo_1", "Se te nubla la vista y te ves tal como eras de pequeño. Oyes una vocetia que entre risas te dice \'¡Al!¡Estoy escondida, ven a buscarme, apúrate!\'<br/>" );
				primitives.GD_CreateMsg ("es", "espejo_2", "Te ves rodeado de un aura de fuego que te cubre. Instintívamente, te pasas las manos por la cabeza, que te notas más cliente de lo normal.<br/>" );
				primitives.GD_CreateMsg ("es", "espejo_3", "El espejo refleja la ventana abierta, a través de la que puede ver el lejano promontorio. Tiemblas al recordar no sólo que los cuerpo de tu querida hermana y tu madre están ahí, sino que ésta última falleció al caer desde ahí.<br/>" );
				primitives.GD_CreateMsg ("es", "espejo_4", "Por un segundo, te viene una imagen de tu recurrente pesadilla. Te ves a ti mismo cubierto del mar y ahogándote en la cueva.<br/>" );
			...
			if (par_c.item1Id == "barrotes") {
				...
				if (primitives.IT_GetAttPropValue (primitives.IT_X("barrotes"), "generalState", "state") == "0") { // bloqueada la entrada
					...
					if (primitives.IT_GetLoc(primitives.IT_X("atizador")) != primitives.PC_X()) {
						primitives.GD_CreateMsg ("es", "falta_atizador", "Los barrotes están muy oxidados y se mueven, pero te convences de que con las manos sólo no vas a poder retirarlos.<br/>");
						primitives.CA_ShowMsg ("falta_atizador");
						return true
					}

					primitives.GD_CreateMsg ("es", "rompes_barrotes", "Gracias al atizador, rompes los barrotes con facilidad, pudiendo ahora entrar en la cuevita.<br/>");
					primitives.CA_ShowMsg ("rompes_barrotes");
					primitives.IT_SetAttPropValue (primitives.IT_X("barrotes"), "generalState", "state", "1")
				} else {
					primitives.GD_CreateMsg ("es", "no_es_necesario_que_insistas", "No es necesario que insistas.<br/>");
	...
	reactions.push ({
		...
		reaction: function (par_c) {
			...
			if ( primitives.PC_IsAt(primitives.IT_X("cuevita")) &&
				...
				// escena final
				primitives.GD_CreateMsg ("es", "epílogo", "<b>Epílogo</b><br/><br/>Días después, organizas una fiesta en la casa de la playa. Asisten Freddie, Vicky con su marido, así como Charlie con varios amigos suyos. Después de comer, beber (y, algunos, fumarse lo que queda de tu planta que ha traído Freddie) en abundancia, les anuncias que estos días en la casa te han hecho renacer, que has quemado tu novela casi acabada y has empezado a escribir una nueva novela.<br/><br/>")
				primitives.CA_ShowMsg ("epílogo");

				primitives.GD_CreateMsg ("es", "DLG_presentación_poema", "Amigos, permitidme leeros este poemita que se me ocurrió ayer, que es en esencia una continuación del poema de Edgan Allan Poe \'Annabelle Lee\' y que creo que va a  jugar un papel importante en mi nueva novela:")
				...
				primitives.GD_CreateMsg ("es", "fin_juego_poema", "El juego ha terminado<hr/>")
				primitives.CA_EndGame("fin_juego_poema")

				return true

			} else {
				primitives.GD_CreateMsg ("es", "no_rompes_collas", "¿Cómo romper algo tan bello que te hizo tu querida hermana el día de su muerte? El sólo hecho de pensarlo te entistrece y te hace soltar lágimas de dolor y pena.<br/>");
		...
		reaction: function (par_c) {
			...
			if (par_c.item1Id == "cerillas") {
				if (primitives.PC_GetCurrentLoc() != primitives.IT_X("salón_comedor")) {
					primitives.GD_CreateMsg ("es", "no_gastar_cerillas", "Haces el amago de encender una, pero te das cuenta de que sólo la estarías desaprovechando aquí.<br/>");
			...
			if (!primitives.IT_IsCarriedOrHere(primitives.IT_X("cerillas"))) {
				primitives.GD_CreateMsg ("es", "sin_cerillas", "No ves nada alrededor con qué encender la chimenea.<br/>");
			...
			if ( primitives.IT_IsAt(primitives.IT_X("manuscrito"), primitives.IT_X("chimenea")) &&
				...
				if (typeof par_c.option == 'undefined') { // phase 1: asking dialog
					...
					if (par_c.option == "apagar_cerilla") {
						primitives.GD_CreateMsg ("es", "hechizo1_abortado", "Con apenas fuerzas, soplas la cerilla. Tu querida hermana te mira con rabia y luego dirige su atención a las tijeras, que salen disparadas hacia ti y que sólo esquivas por los pelos.<br/><br/>La caja de cerillas sale volando de tus manos y ves ante tus ojos cómo se abre y se enciende una cerrilla por sí sola y prende fuego a la chimenea. ¡No! De manera instintiva intentas sacar tu manuscrito de las llamas, pero ya es demasiado tarde.<br/><br/>Pero ése no es suficiente sacrificio para tu hermana, que se sitúa entre la entrada de la casa y las escalera bloqueando tu salida y te señala hacia la cocina y la salida posterior de la casa.<br><br/>");
						...
						primitives.GD_CreateMsg ("es", "fantasma_rompe_móvil", "Al verte dudar, tu querida hermana se enfuerece aún más y vuelve a lanzarte las tijeras, que dan de pleno en tu pecho... justo donde llevabas tu flamante móvil. Nunca creíste que llegaras alegrarte de sentir cómo se rompía su pantalla en mil pedazos.<br>");
			...
			// resto de casos
			primitives.GD_CreateMsg ("es", "fantasma_apaga_cerilla", "Enciendes una cerilla, y cuando vas a prender fuego a la chimenea, notas una corriente de aire extraña que la apaga. Se te herizan los pelos al escuchar una corriente de aire que te recuerda a tu querida hermana cuando jugaba a ser un fantasma.<br/>");
...
usr.turnoCinturon = function() {
	...
	if ( (usr.minutosRestantesMarea() == 0) &&
		...
		primitives.GD_CreateMsg ("es", "desatas_cinturón_con_marea", "Desatas el cinturón de los barrotes antes de que suba más la marea.<br/>");
...
usr.turnoCargador = function() {
	...
	if ( (primitives.IT_GetAttPropValue (indexItemC, "generalState", "state") != "0") &&
		...
		primitives.GD_CreateMsg ("es", "desconectar_cargador", "Desconectas el cargador del móvil.<br/>");
...
usr.incrementar_hora = function(turnos) {
	...
	var minutosHoy = +primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_reloj") + turnos * 5

	if (primitives.PC_IsAt (primitives.IT_X("estudio"))) {
		// si pierde el avión, reseteo del día en el estudio
		if (minutosHoy >= 13 * 60) {
			primitives.GD_CreateMsg ("es", "perdiste_avión", "Ni tú mismo te puedes creer que hayas perdido el avión. Menos mal que no será por vuelos. Llamas a la compañía aérea y reservas otro para la misma hora para mañana. Te pones a trabajar en tu nuevo libro y al llegar la noche comes algo y te vas a la cama. Mañana será otro día... aunque temes la noche y las pesadillas que trae consigo.<br/><br/>");
		...
		if (minutosHoy + minutosPendientesEscribirHoy > 23 * 60) {
			primitives.GD_CreateMsg ("es", "dejarlo_todo_y_escribir", "Te das cuenta de que no has cumplido con tu compromiso de seis horas diarias de revisión diaria de tu novela. Lo dejas todo y te diriges a tu habitación a cumplir con tu deber.<br/>");
			primitives.CA_ShowMsg ("dejarlo_todo_y_escribir");

			if (usr.tiradoEnLaCalleDeNoche ()) {
				primitives.GD_CreateMsg ("es", "el_remordimiento_te_corroe", "Peor aún, te das cuenta de que no podrás cumplir con tu compromiso al no saber cómo volver a casa.<br/>");
				primitives.CA_ShowMsg ("el_remordimiento_te_corroe");
			} else {
				primitives.GD_CreateMsg ("es", "a_dormir_después_de_escribir", "Acabas exhausto después de la sesión de revisión de tu novela. Comes algo y te acuestas.<br/>");
			...
			primitives.GD_CreateMsg ("es", "a_dormir", "<br/>De repente, te das cuenta de qué hora es. Eres muy escrupuloso con tus horarios y a las 23:00 te retiras a casa a dormir, sí o sí. El sueño te vence y te arrastras a la cama. Mañana será otro día.<br/><br/>");
	...
	if (!usr.conLuz()) {
		primitives.GD_CreateMsg ("es", "de_noche_sin_luz", "Es de noche y estás fuera de casa sin iluminación que te guíe.<br/><br/>");
...
usr.reseteoDia = function() {
	...
	if (usr.tiradoEnLaCalleDeNoche ()) {
		primitives.GD_CreateMsg ("es", "tirado_en_la_calle", "De noche, en la calle y sin luz. Te haces un ovillo en cualquier lugar y duermes algo tullido de frío hasta que sale el sol.<br/><br/>");
		...
		if ( (primitives.IT_IsCarried (primitives.IT_X("cargador"))) ||
			...
			primitives.GD_CreateMsg ("es", "antes_dormir_cargas_móvil", "Antes de dormir, estiras el brazo y pones el móvil a cargar.<br/>");
			primitives.CA_ShowMsg ("antes_dormir_cargas_móvil");
		} else {
			primitives.GD_CreateMsg ("es", "antes_dormir_NO_cargas_móvil", "Antes de dormir, estiras el brazo, pero el cargador no está... bueno, ya lo buscarás mañana.<br/>");
	...
	var dias = +primitives.IT_GetAttPropValue (primitives.IT_X("escritor"), "generalState", "state")
	dias++
	primitives.IT_SetAttPropValue (primitives.IT_X("escritor"), "generalState", "state", dias)

	primitives.CA_ShowMsgAsIs ("<hr/>")

	// sueño según el día que sea, o aleatorio pasado el quinto

	primitives.GD_CreateMsg ("es", "sueño2", "Estás durmiendo en tu piso de Nueva York. Te ves desde fuera, flotando encima de ti. Luego te fijas en la foto de la estantería donde estáis tú y tu hermana jugando en la playa. Al lado de la foto está el extraño libro que conseguiste en el anticuario de Marrakech. El libro se abre solo y las páginas comienzan a correr hasta detenerse en una extraña página, donde queda abierto. Finalmente, sales flotando de la habitación, atravesando la ventana.<br/>")
	primitives.GD_CreateMsg ("es", "sueño3", "Entre brumas, ves caras de personas que sufren. Una de ellas te mira y se gira para que la sigas. Atraviesan juntos sitios de extraños de colores y sonidos mareantes y aparecen en el mercado de Marrakech. En trastienda la de un puesto, la figura te señala el libro.<br/>")
	primitives.GD_CreateMsg ("es", "sueño4", "En el puesto del mercado de Marrakech. Te ves desde arriba. Como en una película, la escena muestra cómo observas muchos libros pero no coges ninguno. De repente, la cámara hace un travelling vertigionso al interior del dependiente, cuyos ojos pasan a ser la cámara de la película, que se retira a la parte trasera del puesto, la mano del mercader coge el libro y te lo ofrece.<br/>")
	primitives.GD_CreateMsg ("es", "sueño5", "En el promontorio. Ves cómo tu madre llora desconsoladamente al lado de la tumba de tu querida hermana. De repente, ves cómo fija su mirada en ti, que te retiras hacia atrás, flotando sobre la rompiente mar. Tu madre se acerca a ti y al intentar abrazarte, cae irremediablemente al abismo. Vuelas tras ella, intentando salvarla, pero no puedes hacer nada.<br/>")
	primitives.GD_CreateMsg ("es", "sueño6", "El día en que decidiste viajar a Marruecos. Sentado en tu mesa del salón del piso de Nueva York. Ves como si te hubieran grabado con una cámara web espía, cómo te levantas un momento del ordenador donde mirabas ofertas de viaje. La cámara web enfoca el ordenador y ves cómo aparece una oferta súper barata a Marrakech y Casablanca y cómo, en una segunda página web se abre un artículo sobre Lovecraft y el Necronomicón. La cámara web se retira y te apunta. Observas cómo pones cara de bobo al ver la oferta y el artículo y cómo metes la numeración de tu tarjeta de crédito.<br/>")

	var i
	if (dias < 6) {
		i = dias + 1
	} else {
		i = 1 + primitives.MISC_Random (6)
	}

	primitives.CA_ShowMsg ("sueño" + i)

	primitives.CA_PressKey ("pulsa_avanzar");

	primitives.GD_CreateMsg ("es", "después_sueño2a5", "Pero no, definitivamente estos no son recuerdos tuyos. Sientes a tu querida hermana muy cerca de ti, ¿te estarás volviendo loco?<br/><br/>");
	primitives.CA_ShowMsg ("después_sueño2a5");

	primitives.GD_CreateMsg ("es", "día_s1", "<br/>Comienzas la jornada %s1 desde que tuviste la primera pesadilla.<br/><br/>");
	primitives.CA_ShowMsg ("día_s1", {s1: dias + 1});
	...
	if (pedido.length > 0) {
		primitives.GD_CreateMsg ("es", "recibes_paquete", "<br/>¡Hoy seguro que recibiste el paquete de Freddie! Te diriges a la puerta de la casa, sacas todo el contenido del paquete y lo dejas en el salón comedor.<br/>");
...
usr.gastar_bateria = function(turnos) {
	...
	var decremento = +turnos * (primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_linterna") == "0")? 5: 60
	...
	if (estado == 0) {
		if (estado0 >0) {
			primitives.GD_CreateMsg ("es", "baterías_0", "Te has quedado sin baterías. Te toca apañártelas a la antigua usanza.<br/>");
			primitives.CA_ShowMsg ("baterías_0");
		}
	} else if (estado <= 60) {
		primitives.GD_CreateMsg ("es", "baterías_60", "Te estás quedando sin baterías. Te queda una hora a ritmo normal.<br/>");
		primitives.CA_ShowMsg ("baterías_60");
	} else if (estado <= 120) {
		primitives.GD_CreateMsg ("es", "baterías_120", "Te estás quedando sin baterías. Te quedan menos de dos horas a ritmo normal.<br/>");
...
usr.poema = function() {
	...
	primitives.GD_CreateMsg ("es", "estrofa1", "<br/><br/>De niño pasaba con mi familia<br/>los veranos en una casa junto al mar.<br/>Tenía yo una guapa vecina<br/>cuyo nombre no voy a mencionar<br/>de la que era novio infantil en verano<br/>y a quien el resto del año sólo podía añorar.<br/><br/>");
	primitives.CA_ShowMsg ("estrofa1");
	primitives.GD_CreateMsg ("es", "estrofa2", "Un desgraciado verano,<br/>mi querido amor me fue arrebatado por un golpe de mar<br/>y quedé profundamente conmocionado.<br/>Su tumba iba a visitar <br/>todos los días, ese verano y los siguientes,<br/>en un promontorio junto al mar.<br/><br/>");
	primitives.CA_ShowMsg ("estrofa2");
	primitives.GD_CreateMsg ("es", "estrofa3", "Quince años sin ella<br/>no me pudieron calmar.<br/>Entonces algo sucedió<br/>y me dejaron de mirar<br/>sus ojos reflejados en las estrellas.<br/>Sólo escribir me podía aliviar<br/>y con letras y más letras<br/>no la dejaba de llamar.<br/><br/>");
	primitives.CA_ShowMsg ("estrofa3");
	primitives.GD_CreateMsg ("es", "estrofa4", "En otros ojos y cuerpos<br/>la quise buscar<br/>tanto de ángeles como de demonios.<br/>Pero nunca la pude encontrar<br/>Tanta fue mi desesperación<br/>que casi la llegué a olvidar.<br/><br/>");
	primitives.CA_ShowMsg ("estrofa4");
	primitives.GD_CreateMsg ("es", "estrofa5", "Un día alguien tuvo a bien<br/>una bonita carta quererme enviar.<br/>Era una admiradora a quien en edad podría yo doblar.<br/>Había leído algo mío<br/>que de ella y de mí parecía hablar.<br/>Pero nunca pudimos vernos.<br/>Porque vivía ella lejos de mi casa junto al mar.<br/><br/>");
	primitives.CA_ShowMsg ("estrofa5");
	primitives.GD_CreateMsg ("es", "estrofa6", "Un día, mientras firmaba libros en la ciudad,<br/>A una dulce voz oí rogar<br/>'Me firma, por favor.'<br/>Y al la vista levantar,<br/>los ojos de ella reencontré<br/>Aquellos mismos a quienes tanto amé.<br/>Y fui afortunado de nunca más volverme a separar<br/>de aquellos ojos de más allá de cielo y del mar.<br/><br/>");
...
usr.hablarConPNJ = function(PNJIndex, option, esTelefonico) {
	...
	if (primitives.IT_GetAttPropValue (PNJIndex, "generalState", "state") == "0") {
		...
		if (PNJIndex == primitives.IT_X("hippie")) {
			...
			primitives.GD_CreateMsg ("es", "DLG_hippie_se_presenta", "¡Hola!, ¿qué puedo hacer por usted?... pero espera, ¡tú eres Al, mi viejo amigo de aventuras veraniegas! ¡Cómo me alegro de verte después de tantos años! Yo sigo aquí, con mi música y mi surf... a propósito, mira que app más guapa para saber a qué hora son las mareas")
			primitives.CA_QuoteBegin (PNJId, "DLG_hippie_se_presenta" , [], true );

			// por si tienes el móvil descargado
			if (+primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas") > 0) {
				primitives.GD_CreateMsg ("es", "hippie_app_mareas_móvil_cargado",  "Te coge tu móvil y te la instala sin más.<br/>")
				primitives.CA_ShowMsg ("hippie_app_mareas_móvil_cargado" );
			} else {
				// te presta cargador por la cara
				primitives.GD_CreateMsg ("es", "hippie_app_mareas_presta_cargador",  "Te coge tu móvil y al ver que lo tienes sin baterías, entra en casa con tu móvil y vuelve con el móvil con un poco de carga y la app de mareas instalada.")
				primitives.CA_ShowMsg ("hippie_app_mareas_móvil_cargado" );
				primitives.IT_SetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas", 60)
			}

			primitives.GD_CreateMsg ("es", "DLG_hippie_invita_a_fiesta", "Colega, pásate por casa a partir de las 7 de le noche. Casi siempre hay gente, sexo, drogas y rockandroll a partir de esa hora. Ja, ja, ja. Tráete algo de bebida y yo me encargo del resto. Tengo género de buena calidad.")
			primitives.CA_QuoteBegin (PNJId, "DLG_hippie_invita_a_fiesta" , [], true );

			primitives.GD_CreateMsg ("es", "hippie_se_despide", "<br/>Sin darte mucho margen, tu amigo de la infancia se mete en casa («a dormir la mona», dice), no sin antes dejarte su número de móvil.<br/>");
			...
			if ( (primitives.IT_GetLoc(primitives.IT_X("libro_magia")) == primitives.IT_X("estudio")) ||
				...
				primitives.GD_CreateMsg ("es", "libro_en_estudio", "«¡Mierda!», recuerdas que no metiste el libro en la maleta; «quizás el bueno de Freddie me lo pueda enviar por mensajería.»<br/>");
	...
	// to-do: no se llega aquí y nunca recuperas el cargador!

	if (typeof option == 'undefined') { // phase 1: asking dialog

		var menu = []

		// si cargador en estudio y estás sin baterías, habilita pedir uno prestado
		if ( (+primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_pilas") == 0) &&
			...
			primitives.GD_CreateMsg ("es", "pedir_cargador", "Pedir prestado un cargador");
		...
		var personaje = (PNJId=="embarazada"?"Vicky":"Charlie")
		primitives.CA_ShowMenu (menu, {type: 'img', isLocal: true, src:"NPC_" + personaje + ".jpg", txt:personaje} );
		// continuation in state == 0, phase 2
	...
	if (option == "pedir_cargador") {
		primitives.GD_CreateMsg ("es", "DLG_pedir_cargador_reacción", "Venga, va, pero no te lo olvides por ahí, que me da que eres un poco despistado.");
		...
		if (PNJIndex == primitives.IT_X("embarazada")) {
			...
			primitives.GD_CreateMsg ("es", "o1_te_devuelve_el_saludo", "%o1 te devuelve el saludo.");
...
usr.turnoPNJ = function (PNJIndex) {
	...
	if (timbre > 0) {
		...
		if (timbre == 0) {
			if (primitives.IT_IsHere(PNJIndex)) {
				primitives.GD_CreateMsg ("es", "DLG_se_despide", "Bueno, tengo mucho, lío. Ya hablamos.")
				primitives.CA_QuoteBegin (PNJId, "DLG_se_despide" , [], true );
				primitives.GD_CreateMsg ("es", "se_va_o1", "%o1 se va<br/>");
	...
	if (primitives.IT_GetLoc(PNJIndex) ==  primitives.PC_GetCurrentLoc()) {
		// nada que decir
		primitives.GD_CreateMsg ("es", "o1_te_mira", "%o1 te mira, esperando ver qué quieres.")
...
usr.turnoCuevita = function () {
	...
	primitives.GD_CreateMsg ("es", "la_marea_sube", "La marea sube y cada ves oyes el mar más cerca. Puede ser peligroso permanecer mucho tiempo aquí.<br/>");
	primitives.CA_ShowMsg ("la_marea_sube");

	// venció el tiempo de la marea -> fuera de la cueva, de manera automática
	if (usr.minutosRestantesMarea() <= 0) {
		primitives.GD_CreateMsg ("es", "el_agua_entra", "El agua empieza a entrar en la cueva. Te asustas y te afanas por salir.");
		...
		if (primitives.IT_GetAttPropValue (primitives.IT_X("cinturón"), "generalState", "state") == "0")  {
			...
			if (primitives.MISC_Random (100) < probabilidadMuerte) {
				primitives.GD_CreateMsg ("es", "el_agua_entra_y_mueres", "Sin embargo, resbalas y caes atrás. Tocas los barrotes pero no consigues asirlos por las algas pegadas. Un corrimiento de area aprisona tus pies y sólo puedes ser testigo de ver cómo cada vez entra más agua. Cuando por fin el agua te sobrepasa y no te deja respirar, crees ver la figura de tu querida hermana, que te sonríe y te ofrece la mano para que vayas con ella a un círculo de luz.<br/><br/><hr/>");
				primitives.CA_ShowMsg ("el_agua_entra_y_mueres");

				primitives.GD_CreateMsg ("es", "fin_juego_asfixia_sin_cinturón", "El juego ha terminado, pero ¿qué habría pasado si hubieras atado una cuerda o similar a los barrotes para poder salir con ellos?")
				primitives.CA_EndGame("fin_juego_asfixia_sin_cinturón")
				return true
			} else {
				primitives.GD_CreateMsg ("es", "el_agua_entra_y_sales", "Afortunadamente, consigues asir los barrotes sin resbalarte, pero te apuntas mentalmente atar algo a los barrotes para no volver a llevarte este susto.<br/><br/>")
				primitives.CA_ShowMsg ("el_agua_entra_y_sales");

			}
		} else {
			primitives.GD_CreateMsg ("es", "el_agua_entra_y_sales_con_cinturón", "Agarras el cinturón y tirando de él consigues salir sin demasiados problemas<br/><br/>")
	...
	if (primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "app_linterna") == "0")  {
		primitives.GD_CreateMsg ("es", "cueva_se_oscurece", "Estar sin luz en la cueva es superior a ti. A duras penas recoges tus cosas y sales a trompicones.<br/><br/>");
...
usr.turnoCestaTelePapeo = function () {
	...
	if ( (estado_hora.minutosDia > 900) &&
		...
		primitives.GD_CreateMsg ("es", "comes_con_ansia", "¡La hora que es y aún sin comer! Devoras con ansia el contenido de la cesta de TelePapeo.<br/><br/>");
...
usr.turnoFantasma = function () {
	...
	if (estado == 1) {
		if ( !primitives.IT_IsAt(primitives.IT_X("setas"), primitives.IT_X("limbo")) &&
			...
			primitives.GD_CreateMsg ("es", "fantasma_te_incita_a_hechizo", "Oyes una voz que te dice \'haz el hechizo, hazlo, ¡hazlo!\'");
    ...
    if (!primitives.PC_IsAt(primitives.IT_X("cuevita")))   {
		...
		primitives.GD_CreateMsg ("es", "fantasma_te_mira_enfadado", "Tu querida hermana brilla en la noche y te señala el camino.<br/><br/>");
		...
		if (contadorLocalidad > 4 ) {
			...
			if (nuevaLoc != -1) {
				...
				primitives.GD_CreateMsg ("es", "fantasma__te_empuja_a_o1", "Tu querida hermana no soporta la espera y te arrastra a %o1.<br/><br/>");
			...
			if (primitives.PC_IsAt(primitives.IT_X("camino_playa"))) {
				primitives.GD_CreateMsg ("es", "fantasma_te_apunta_dos_direcciones", "Con cada mano apunta dos direcciones, como preguntándote dónde quieres morir y reunirte con ella: si en el promontorio o en la cueva de la caleta.");
		...
		if (estado < 5) {
			...
			primitives.GD_CreateMsg ("es", "te_asfixias_con_fantasma", "La marea sube irremediablemente. El agua te cubre y empiezas a experimentar la misma asfixia que tu querida hermana el día en que murió. Tu querida hermana te mira complacida, pronto te reunirás con ella.");
			...
			primitives.GD_CreateMsg ("es", "mueres_asfixiado_con_fanstama", "Tu querida hermana se acerca y te toma por la mano. La acompañas a un círculo luminoso, al mirar atrás ves tu cuerpo ahogado en la cueva.<hr/>");
			primitives.CA_ShowMsg ("mueres_asfixiado_con_fanstama");

			primitives.GD_CreateMsg ("es", "fin_juego_asfixia", "El juego ha terminado, pero quizás podría haber sido de otra manera si no te hubieras sentido tan apegado al collar de tu querida hermana.")
...
usr.turnoCaleta = function () {
	...
	if (usr.minutosRestantesMarea() <= 0) {
		primitives.GD_CreateMsg ("es", "marea_alta", "La marea cubre la entrada a la cueva donde toco ocurrió.<br/><br/>");
		primitives.CA_ShowMsg ("marea_alta");
	} else if (usr.conLuz()) {
		primitives.GD_CreateMsg ("es", "marea_baja", "La marea está muy baja y permite ver la entrada a la cueva donde toco ocurrió.<br/><br/>");
...
usr.menuTelefonoFreddie = function (option) {
	...
	if (typeof option == 'undefined') { // phase 1: asking dialog
		...
		if (pedido.length == 0) {
			...
			primitives.GD_CreateMsg ("es", "Freddie_solo_saludar", "Simplemente lo saludas y te aseguras de que va todo bien por el estudio.");
			...
			primitives.GD_CreateMsg ("es", "Freddie_cancelar_paquete", "Te lo piensas mejor, y al final no le pides nada y te despides depués de asegurarte de que va todo bien por el estudio.");
			primitives.GD_CreateMsg ("es", "Freddie_fin_paquete", "Te despides, insistiéndole que te envíe el paquete urgente, que ya se lo pagarás.");
		...
		if (pedido.indexOf ("cargador") < 0) { // no está en el pedido
			if ( (primitives.IT_GetLoc(primitives.IT_X("cargador")) == primitives.IT_X("estudio")) ||
				...
				primitives.GD_CreateMsg ("es", "Freddie_cargador", "Le pides el cargador.");
		...
		if (pedido.indexOf ("foto2") < 0) { // no está en el pedido
			if ( (primitives.IT_GetLoc(primitives.IT_X("foto2")) == primitives.IT_X("estudio")) ||
				...
				primitives.GD_CreateMsg ("es", "Freddie_foto", "Le pides la foto tuya con tu querida hermana.");
		...
		if (pedido.indexOf ("libro_magia") < 0) { // no está en el pedido
			if ( (primitives.IT_GetLoc(primitives.IT_X("libro_magia")) == primitives.IT_X("estudio")) ||
				(primitives.IT_GetLoc(primitives.IT_X("libro_magia")) == primitives.IT_X("estantería")) ) {
				primitives.GD_CreateMsg ("es", "Freddie_libro", "Le pides el libro ilustrado.");
		...
		primitives.CA_ShowMenu (menu,  {type: 'img', isLocal: true, src:"NPC_Freddie.jpg", txt:'Freddie'} ); // continuation in state == 0, phase 2
		...
		if (option == "Freddie_solo_saludar") {
			...
			*/
			...
			primitives.GD_CreateMsg ("es", "Freddie_solo_saludar_reacción", "Freddie te dice que va todo bien y cuelga.");
			...
			primitives.GD_CreateMsg ("es", "Freddie_cancelar_paquete_reacción", "Freddie te dice que va todo bien y cuelga.");
			primitives.CA_ShowMsg ("Freddie_cancelar_paquete_reacción");
			primitives.IT_SetAttPropValue (primitives.IT_X("cestaFreddie"), "generalState", "state", "[]") // resetea cesta
			return
			primitives.IT_SetAttPropValue (primitives.IT_X("móvil"), "apps", "llamada", "") // fin de la llamada
		} else if (option == "Freddie_fin_paquete") {
			primitives.GD_CreateMsg ("es", "Freddie_fin_paquete", "Freddie te dice te lo enviará ahora mismo.");
			...
			primitives.GD_CreateMsg ("es", "Freddie_pedido_reacción", "Freddie te dice que te lo enviará mañana por correo y que si quieres algo más.");
...
usr.menuTelePapeo = function (option) {
	...
	if (primitives.IT_GetLoc(primitives.IT_X("cestaTelePapeo")) != primitives.IT_X("limbo")) {
		primitives.GD_CreateMsg ("es", "pedido_ya_realizado", "En realidad, con lo que ya pediste te puedes apañar por hoy, así que te resignas y cuelgas.<br/>");
	...
	if (typeof option == 'undefined') { // phase 1: asking dialog
		...
		primitives.GD_CreateMsg ("es", "TelePapeo_cancelar_paquete", "Te lo piensas mejor, y al final no pides nada.");
		menu.push ({id:"TelePapeo_cancelar_paquete", msg:"TelePapeo_cancelar_paquete"})

		if (pedido.length > 0) {
			primitives.GD_CreateMsg ("es", "TelePapeo_fin_paquete", "Cierras el pedido, que recibirás en la puerta de la casa.");
			menu.push ({id:"TelePapeo_fin_paquete", msg:"TelePapeo_fin_paquete"})
		}

		for (var i=0; i<productos.length; i++) {
			primitives.GD_CreateMsg ("es", "TelePapeo_prod_" + productos[i].id, "Añadir " + productos[i].id + " al pedido");
			menu.push ({id:"TelePapeo_prod_" + productos[i].id, msg:"TelePapeo_prod_" + productos[i].id})
		}

		primitives.CA_ShowMenu (menu,  {type: 'img', isLocal: true, src:"TelePapeo.jpg", txt:'TelePapeo'} );
		...
		if (option == "TelePapeo_cancelar_paquete") {
			primitives.GD_CreateMsg ("es", "TelePapeo_cancelar_paquete_reacción", "Cancelas el pedido a TelePapeo.<br/>");
			...
			primitives.GD_CreateMsg ("es", "TelePapeo_fin_paquete", "TelePapeo te dice te lo enviará ahora mismo.<br/>");
			...
			if ((productos[producIndex].id != "whisky") ||
				...
				primitives.GD_CreateMsg ("es", "TelePapeo_pedido_reacción", "Lo añades al pedido");
				...
				primitives.GD_CreateMsg ("es", "whisky_antes_de_tiempo", "En realidad eres más de ron que de whisky, así que lo descartas.");
...
usr.agregarDialogos = function (PNJIndex, menu) {
	...
	primitives.GD_CreateMsg ("es", "saludar_embarazada", "Hola, Vicky, ¿qué tal estás?");
	primitives.GD_CreateMsg ("es", "saludar_hippie", "Hola compadre, ¿cómo va todo?");
	menu.push ({id:"saludar_" + PNJId , msg:"saludar_" + PNJId})

	if (PNJId == "embarazada") {

			// primitives.GD_CreateMsg ("es", "Vicky_hablar_de_tu_libro", "Vicky_hablar_de_tu_libro");
		// primitives.GD_CreateMsg ("es", "Vicky_hablar_de_tu_hermana", "Vicky_hablar_de_tu_hermana");
		// primitives.GD_CreateMsg ("es", "Vicky_hablar_de_tu_madre", "Vicky_hablar_de_tu_madre");
		// primitives.GD_CreateMsg ("es", "Vicky_hablar_de_la_casa", "Vicky_hablar_de_la_casa");
		// primitives.GD_CreateMsg ("es", "Vicky_hablar_de_infancia", "Vicky_hablar_de_infancia");
		...
		if (primitives.IT_GetAttPropValue (PNJIndex, "generalState", "state") == "2") {
			primitives.GD_CreateMsg ("es", "Vicky_preguntar_por_embarazo", "Preguntar a Vicky por embarazo");
			...
			if ( (primitives.IT_IsCarried(primitives.IT_X("cestaTelePapeo"))) &&
				...
				primitives.GD_CreateMsg ("es", "Vicky_invitar_a_comer", "Te invito a comer algo");
...
usr.menuTelefonoVicky = function (option) {
	...
	if (primitives.IT_IsHere(PNJIndex) ) {
		primitives.GD_CreateMsg ("es", "embarazada_dice_estoy_aquí", "¿No ves que estoy aquí, para qué me llamas?");
	...
	if ( !usr.disponibilidadPNJ ("embarazada") ) {
		...
		primitives.GD_CreateMsg ("es", "embarazada_dice_no_puedo_hablar_ahora", "Ahora no te puedo atender, lo siento. Estaré libre de 10:00 a 17:00. Llamame entonces. Besos.");
		...
		primitives.GD_CreateMsg ("es", "y_cuelga", " Y cuelga<br/>");
	...
	if (typeof option == 'undefined') { // phase 1: asking dialog
		...
		primitives.GD_CreateMsg ("es", "embarazada_dice_qué_quieres", "Hola, querido Al, ¿qué puedo hacer por ti?");
		...
		primitives.CA_ShowMenu (menu, {type: 'img', isLocal: true, src:"NPC_Vicky.jpg", txt:'Vicky'} );
		// continuation in state == 0, phase 2
...
usr.reaccionesVicky = function (option, esTelefonico) {
	...
	primitives.GD_CreateMsg ("es", "Y_cuelga", "Y cuelga el teléfono.<br/>");

	if (option == "saludar_embarazada") {

		primitives.GD_CreateMsg ("es", "saludar_embarazada_reacción", "Le preguntas qué tal está, a ver si te cuenta algo.<br/>");
		...
		if (primitives.IT_GetAttPropValue (PNJIndex, "generalState", "state") != "4") {  // aún no ha sido la escena de la servilleta
			primitives.GD_CreateMsg ("es", "DLG_Vicky_responde_saludo", "Todo bien, majete. A ver si nos vemos.");
			primitives.CA_QuoteBegin ("embarazada",  "DLG_Vicky_responde_saludo", [], true );
		} else {
			primitives.GD_CreateMsg ("es", "DLG_Vicky_no_mas_chocolate", "Al, júrame que no me dejarás volver a comer chocolate. Sentí como un demonio en el estómago.");
		...
		primitives.GD_CreateMsg ("es", "Vicky_preguntar_por_embarazo_reacción", "Con mucho tacto masculino, le preguntas a Vicky si está embarazada de gemelos.");
		primitives.CA_ShowMsg ("Vicky_preguntar_por_embarazo_reacción");

		primitives.GD_CreateMsg ("es", "DLG_confirma_embarazo_gemelos", "¿Tan gorda estoy? Sí, de gemelos. Te iba a decir que tengo un antojito de chocolate y tenías algo, pero como veo que me ves como una foca, casi que me abstengo.");
		...
		if (!usr.paqueteContiene (primitives.IT_X("cestaTelePapeo"), "chocolate")) {
			primitives.GD_CreateMsg ("es", "invitas_sin_chocolate", "Vicky se planta contigo en casa y da buena cuenta de la comida, pero en cuanto ve que no tienes chocolate, se va apresuradamente no sin antes soltarte que \'a ver si la próxima vez me invitas a chocolate, vecino.\'.<br/><br/>");
			...
			usr.incrementar_hora(9) // 45 minutos

		} else {
			// escena de merienda de chocolate y servilleta

			primitives.GD_CreateMsg ("es", "escena_merienda_con_Vicky_1", "A los pocos minutos, tienes a Vicky en casa. Durante unas horas os ponéis al día sobre vuestras vidas. Te habla sobre la aburrida vida de la villa costera y cómo te ha echado de menos.");
			primitives.CA_ShowMsg ("escena_merienda_con_Vicky_1");

			primitives.GD_CreateMsg ("es", "DLG_embarazada_habla_1", "Querido Al, ¡qué raro se me hace verte sin tu querida hermana al lado! Después de aquello, tu madre te envió lejos y casi no tuvimos tiempo ni de despedirnos. Pensarás que estoy loca, pero cuando he ido a dejar flores a las tumbas de tu hermana y tu malograda madre, las brumas de la costa siempre parecen tomar sus formas, como llamándome a reunirme con ellas. Una vez casi me despeño en el promontorio.");
			primitives.CA_QuoteBegin ("embarazada",  "DLG_embarazada_habla_1", [], true );

			// escena de la posesión
			primitives.GD_CreateMsg ("es", "escena_merienda_con_Vicky_2", "<br/>Vicky da buena cuenta del helado de chocolate. Comienza comiendo comedidamente, pero luego devora como una posesa... tan posesa que con la boca llena de chocolate te mira con los ojos en blanco y te dice:<br/>");
			primitives.CA_ShowMsg ("escena_merienda_con_Vicky_2");

			primitives.GD_CreateMsg ("es", "DLG_embarazada_habla_2", "Querido hermano, pronto estaremos juntos. Necesitarás sangre de nuestra nueva madre para realizar el hechizo de reunión. Aquí te dejo un poco");
			primitives.CA_QuoteBegin ("embarazada",  "DLG_embarazada_habla_2", [], true );

		    primitives.CA_PressKey ("pulsa_avanzar");

			primitives.GD_CreateMsg ("es", "escena_merienda_con_Vicky_3", "<br/><br/>Observas atónito cómo Vicky se muerde ligeramente el labio y recoge unas gotas de sangre con una servilleta, que te tira por encima de la mesa.	Acto seguido, como si hubiera sido un mal sueño, Vicky toma el control de su cuerpo y sigue hablando como si nada:<br/>");
			primitives.CA_ShowMsg ("escena_merienda_con_Vicky_3");

			primitives.GD_CreateMsg ("es", "DLG_embarazada_habla_3", "¡Qué tonta, me he mordido de tantas ganas que tenía de chocolate!");
			primitives.CA_QuoteBegin ("embarazada",  "DLG_embarazada_habla_3", [], true );

			primitives.GD_CreateMsg ("es", "escena_merienda_con_Vicky_4", "<br/><br/>Poco después, Vicky regresa a su casa y tú sigues con tus rutinas.<br/><br/>");
			...
			usr.incrementar_hora(24) // 2 horas
		}

	} else {

		primitives.GD_CreateMsg ("es", "Vicky_no_tiene_qué_decir", "Vicky no tiene mucho que decirte.");
...
usr.menuTelefonoCharlie = function (option) {
	...
	// si está aquí -> ¿para qué me llamas si estoy aquí?
	if (primitives.IT_IsHere(PNJIndex) ) {
		primitives.GD_CreateMsg ("es", "hippie_dice_estoy_aquí", "¿No ves que estoy aquí, para qué me llamas?");
	...
	if ( !usr.disponibilidadPNJ ("hippie") ) {
		primitives.GD_CreateMsg ("es", "hippie_dice_no_puedo_hablar_ahora", "Po favó, déjame \'rmir un poito más.");
	...
	if (primitives.IT_GetAttPropValue (primitives.IT_X("móvil"), "apps", "llamada") == "2") {
		...
		primitives.GD_CreateMsg ("es", "hippie_saturado_de_fiestas", "Hermano, yo no tengo tu aguante. Creo que dejaré las fiestas por un tiempo y me dedicaré a surferar. No me llames por un tiempo, que me tengo que recuperar.");
	...
	primitives.GD_CreateMsg ("es", "Charlie_te_invita_a_fiesta", "Entre un montón de ruido de fondo de música y risas, sólo le alcanzas a entender que necesita más whisky, que te vengas al fiestón y que te quiere mucho.<br/>");
